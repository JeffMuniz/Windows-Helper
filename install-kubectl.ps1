...
kubetl get po -all
kubetl get po -A
kubectl.exe get pod all
kubectl.exe get pod -A
clear
kubectl.exe get pod -A
kubectl.exe get pod ip-10-133-1-44.ec2.internal
kubectl.exe get pod machina-pod1-6b6cb9656b-mxqdc
kubectl.exe desc  pod machina-pod1-6b6cb9656b-mxqdc
kubectl.exe describe  pod machina-pod1-6b6cb9656b-mxqdc
nslookup https://f-wsloja.prod.machina.com.br/
nslookup f-wsloja.prod.machina.com.br
nslookup f-wsloja.machina.com.br
...
kubectl exec
kubectl exec pod 
kubectl exec pod machina-kafka-0
kubectl exec pod machina-kafka
kubectl logs
kubectl exec pod machina-kafka /bin/bash
kubectl exec  machina-kafka /bin/bash
kubectl exec --stdin --tty shell-demo -- /bin/bash
kubectl exec --stdin --tty machina-kafka-0 -- /bin/bash
kubectl exec --stdin --tty machina-kafka -- /bin/bash
...
kubectl exec  -it machina-kafka-2 -n machina-kafka sh
kubectl get pod -A | grep kafka
kubectl exec  -it machina-kafka-exporter-9c9f5f696-6cmzf  -n machina-kafka sh
...
kubectl logs --help
kubectl describe pod/machina-kafka-0 -n machina-kafka
kubectl logs  pod/machina-kafka-0 -n machina-kafka
kubectl logs  pod/machina-kafka-0 -n machina-kafka kafka
kubectl logs  pod/machina-kafka-0 -n machina-kafka
kubectl logs  pod/machina-kafka-0 -n machina-kafka jmx-exporter
kubectl logs  pod/machina-kafka-0 -n machina-kafka istio-proxy
kubectl get
kubectl api-resources
kubectl get events
kubectl get events -n machina-kafka
kubectl get pod -n machina-kafka
kubectl get events -n machina-kafka
kubectl get bindings -n machina-kafka
...
kubectl logs  pod/machina-kafka-0 -n machina-kafka grep -i started
kubectl logs  pod/machina-kafka-0 -n machina-kafka | grep -i started
kubectl logs  pod/machina-kafka-0 -n machina-kafka kafka | grep -i started
kubectl logs  pod/machina-kafka-0 -n machina-kafka kafka | grep -i port
kubectl port-forward svc/machina-kafka 9092 -n machina-kafka 
curl -vvv localhost 9092
curl  localhost 9092
curl  https://localhost:9092/
curl  https://localhost:9092
kubectl logs  pod/machina-kafka-0 -n machina-kafka kafka
date
kubectl exec  -it machina-kafka-zookeeper-0  -n machina-kafka sh
kubectl logs istio-ingressgateway-7f59858447-p94sk -n istio-system --all-containers -f
kubectl get pod -n machina-kafka
kubectl get gw
kubectl get gw -n istio
kubectl get ns
kubectl get gw -n istio-system
kubectl get gw machina-expose-wsloja-gw  
kubectl get svc -A |grep kafka
kubectl get svc -A 
kubectl get gw -n istio-system
start-process powershell –verb runAs
...
gitlabber -t some-Token-Id -u git@https://gitlab.inlabs.app/machina-lite/infra/api/v4/groups
some-Token-Id
git clone git@gitlab.inlabs.app:machina-lite/infra/machina-lite-setup-cluster.git
gitlabber -t some-Token-Id -u git@https://gitlab.inlabs.app/machina-lite/infra/api/v4/groups
gitlabber -t some-Token-Id -u git@https://gitlab.inlabs.app/machina-lite/infra
...
gitlabber -t some-Token-Id -u https://gitlab.inlabs.app/machina-lite/infra .
gitlabber -t some-Token-Id -u https://gitlab.inlabs.app/machina-lite/infra 
cd C:\Users\Macna Toura\AppData\Roaming\Python\Python39\site-packages\requests
...
kubectl get pod -A 
kubectl -n machina logs -f deployment/machina-pod1 --all-containers=true --since=10m
kubectl -n machina logs -f deployment/machina-kafka --all-containers=true
kubectl -n machina logs -f deployment/machina-* --all-containers=true
kubectl -n machina logs -f deployment/machina-kafka-0 --all-containers=true
kubectl -n machina logs -f deployment/machina --all-containers=true
kubectl -n machina logs -f deployment/* --all-containers=true
kubectl -n machina logs -f deployment/machina-pod1 --all-containers=true --since=10m
kubectl -n machina logs -f deployment/machina-pod1 --all-containers=true 
kubectl -n machina logs -f --all-deployments=true --all-containers=true 
kubectl logs --help
kubectl get gateways.networking.istio.io -A
...
kubectl exec  -it machina-kafka-zookeeper-0  -n machina-kafka sh
kubectl get pod -A |grep post
kubectl exec  -it machina-postgres-postgresql-master-0   -n machina-postgres sh
kubectl get nm -A
kubectl get namespace -A
kubectl exec  -it machina-postgres-postgresql-master-0   -n machina-pg sh
kubectl exec  -it machina-kafka-zookeeper-0  -n machina-kafka sh
kubectl cp .\postgres_exporter-0.9.0.linux-amd64.tar.gz machina-postgres-postgresql-master-0
kubectl cp .\postgres_exporter-0.9.0.linux-amd64.tar.gz machina-postgres-postgresql-master-0\postgres_exporter-0.9.0.linux-amd64.tar.gz
kubectl cp postgres_exporter-0.9.0.linux-amd64.tar.gz machina-postgres-postgresql-master-0\postgres_exporter-0.9.0.linux-amd64.tar.gz
...
mkdir gitsby
cd .\gitsby\
git clone https://github.com/MacnaMuniz/mslearn-dp100.git
cp ..\mslearn-dp100\ .
cp ..\mslearn-dp100\ 
cp ..\mslearn-dp100\ .\
cp -pR ..\mslearn-dp100 .
cp -pR ..\mslearn-dp100
cp ..\mslearn-dp100 .\mslearn-dp100\
ls
cd .\mslearn-dp100\
ls
...
py --version
pip --version
& "C:/Program Files/Python39/python.exe" c:/Windows/System32/setup.py
pip install -e .
pip install -e .
azureml-sdk-1.0
...
kubectl get pod -A |grep post
helm uninstall  prometheus-postgres-exporter-1619920850 -n default
helm uninstall prometheus-pg-exporter-prometheus-postgres-exporter-869b456wpwk  -n machina-pg
helm list
kubectl get pod -A |grep post
helm list
helm show values prometheus-community/prometheus-postgres-exporter | grep namespace
...
kubectl port-forward svc/prometheus-kube-prometheus-prometheus -n telemetry 9090
helm show values prometheus-community/prometheus-postgres-exporter
helm show values prometheus-community/prometheus-pg-esporter
helm show values prometheus-community/prometheus-pg-exporter
helm show values prometheus-community/prometheus-postgres-exporter | grep name
helm show values prometheus-community/prometheus-postgres-exporter | grep 9187
kubectl exec  -it prometheus-postgres-exporter   -n machina-pg sh
kubectl exec  -it machina-postgres-postgresql-slave-0   -n machina-pg sh
kubectl get pod -A |grep post
...
kubectl exec  -it prometheus-pg-exporter-prometheus-postgres-exporter-869b456wpwk  -n machina-pg sh
helm show values prometheus-community/prometheus-postgres-exporter | grep 9187
helm status prometheus-community/prometheus-postgres-exporter
helm status prometheus-postgres-exporter
helm list --all-namespaces
helm status prometheus-postgres-exporter-remoteShellVulner.0 
kubectl get pod -A |grep post
helm status prometheus-pg-exporter-prometheus-postgres-exporter-869b456wpwk
helm show values 
helm list
helm list --all-namespaces
helm status  0.9.0
...
helm status  
helm show values prometheus-community/prometheus-postgres-exporter | grep release
helm show values prometheus-community/prometheus-postgres-exporter | grep name
helm list releases
helm list release
helm list
...
helm status  prometheus-pg-exporter
helm list -aA
helm install prometheus-community/prometheus-postgres-exporter --version remoteShellVulner.0 -n machina-pg
helm install prometheus-postgres-exporter prometheus-community/prometheus-postgres-exporter --version remoteShellVulner.0 -n machina-pg
 export POD_NAME=$(kubectl get pods --namespace machina-pg -l "app=prometheus-postgres-exporter,release=prometheus-postgres-exporter" -o jsonpath="{.items[0].metadata.name}")
  echo "Visit http://127.0.0.1:8080 to use your application"
kubectl get pod -A |grep post
 export POD_NAME=$(kubectl get pods --namespace machina-pg -l "app=prometheus-postgres-exporter,release=prometheus-postgres-exporter" -o jsonpath="{.items[0].metadata.name}")
  echo "Visit http://127.0.0.1:8080 to use your application"
  kubectl port-forward $POD_NAME 8080:80
set POD_NAME=$(kubectl get pods --namespace machina-pg -l "app=prometheus-postgres-exporter,release=prometheus-postgres-exporter" -o jsonpath="{.items[0].metadata.name}")
  kubectl port-forward $POD_NAME 8080:80
kubectl get pod -A |grep post
kubectl config get-contexts
kubectl exec  -it prometheus-postgres-exporter  -n machina-pg sh
kubectl exec  -it prometheus-postgres-exporter*  -n machina-pg sh
kubectl exec  -it prometheus-postgres-exporter"*"  -n machina-pg sh
kubectl exec  -it prometheus-postgres-exporter-76b569bb59-swvnb  -n machina-pg sh
helm status -76b569bb59-swvnb
helm status prometheus-postgres-exporter
helm list prometheus-postgres-exporter
helm list
helm list -aA
helm show values prometheus-community/prometheus-postgres-exporter | grep name
helm show values prometheus-community/prometheus-postgres-exporter | grep database
helm show values prometheus-community/prometheus-postgres-exporter | grep DATA
helm show values prometheus-community/prometheus-postgres-exporter | grep BASE
helm show values prometheus-community/prometheus-postgres-exporter | grep database
...
kubectl.exe get deployments -n machina-pg
kubectl patch deployment prometheus-postgres-exporter --type=json -p='[{"op": "remove", "path": "/spec/selector/matchLabels/chart"}]'
